...
# Modified August 2024 by R. Gacesa for RUG/UMCG Habrok cluster
#
####################################################################
#
...
# ============================================
# LOAD MODULES [HABROK, RUG cluster]
# ============================================
...
echo "DATABASES:"
echo ${diamond_database}
echo ${diamond_subsys_db}
echo ${RefSeq_db}
echo ${Subsys_db}
...
do
    f2=`echo $f | awk -F "_R1." '{print $1 "_R2." $2}'`
    echo "processing ${f} and ${f2}"
    #f2=${f/_1.fastq.gz/_2.fastq.gz}
    #out_path=${f/_1.fastq.gz/.cleaned}
