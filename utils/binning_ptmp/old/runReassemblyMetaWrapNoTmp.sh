...
metaWRAP reassemble_bins -b ${BINS_IN_CLN} -1 ${FQ1} -2 ${FQ2} -l $MIN_CONTIG_LT --strict-cut-off 2 --permissive-cut-off 5 -o ${OUT} -t 8 -c $COMPLETENESS -x $CONTAMINATION 2>&1 | tee JOB_OUT/__reassembly.log
echo "metaWRAP reassemble_bins -b ${BINS_IN_CLN} -1 ${FQ1} -2 ${FQ2} -l $MIN_CONTIG_LT --strict-cut-off 2 --permissive-cut-off 5 -o ${OUT} -t 8 -c $COMPLETENESS -x $CONTAMINATION 2>&1 | tee JOB_OUT/__reassembly.log"
