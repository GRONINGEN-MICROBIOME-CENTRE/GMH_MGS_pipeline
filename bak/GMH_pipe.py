            ...
            if cfg.get('PIPELINE','doMeta4',fallback='0') == '1':
                with open(smpl+'_pM4.sh','w') as oF:
                    ...
                    if useLS:
                        ...
                        # run it
                        oF.write(cfg.get('METAPHLAN4','metaphlan')+' '+smplCleanMrgd+' --input_type fastq --nproc '+cfg.get('METAPHLAN4','threads') + ' -o '+sampleDataF+'/metaphlan4/'+smpl+'_metaphlan.txt'+' --tmp_dir '+sampleDataF+'/metaphlan4_tmp' + ' --bowtie2db '+cfg.get('METAPHLAN4','metaphlanDB') + ' ' + cfg.get('METAPHLAN4','metaphlanOptions')+strainPhlanOpts+' 2>&1 | tee '+smpl+'/'+smpl+'_metaphlan4.log'+'\n')
                        # collect data
